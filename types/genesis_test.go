...
func TestGenesisBad(t *testing.T) {
	...
	testCases := [][]byte{
		...
		[]byte(
			`{"validators":[{"pub_key":{"value":"ALvInmwdEH3Z6i6R5mSVIomm18NtqPnlejcqESofdwoa"},"power":"10","name":""}]}`,
		),
		// missing chain_id
		[]byte(
			`{"validators":[` +
				`{"pub_key":{` +
				`"type":"tendermint/PubKeySm2","value":"ALvInmwdEH3Z6i6R5mSVIomm18NtqPnlejcqESofdwoa"` +
				`},"power":"10","name":""}` +
				`]}`,
		),
		// too big chain_id
