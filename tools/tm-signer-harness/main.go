...
func extractKey(tmhome, outputPath string) {
	...
	pkb := algo.GetPrivKeyBytes(fpv.Key.PrivKey)
	if err := ioutil.WriteFile(internal.ExpandPath(outputPath), pkb[:algo.GetPrivKeySize()], 0644); err != nil {
